...
subscription_key <- "9792eb7c42614552a2a27b7fed9b57bb"
...
# taxon_ids <- c(237370)
# 
# result <- get_dyntaxa_taxonomy(taxon_ids, subscription_key)



# getTaxonParentIds <- function(taxon_ids, subscription_key) {
...
#     parsed_result <- c(fromJSON(result)$taxonIds, taxon_ids)
...
# get_taxa_data <- function(taxon_ids, subscription_key) {
...
#   taxa <- data.frame()
#   
#   for (ids in taxon_ids) {
#     single <- unique(ids)
#     
#     taxa_i <- data.frame()
#     
#     for (id in single) {
#       if (id %in% taxa_i$taxonId) {
#         selected <- taxa_i %>%
#           filter(taxonId == id)
...
#       } else {
#         taxa_ix <- get_dyntaxa_taxonomy(id, subscription_key)
#         taxonId <- taxa_ix$taxonId
#         name <- taxa_ix$names %>%
...
#       taxa_i <- bind_rows(
#         taxa_i,
#         data.frame(taxonId, name, rank, hierarchy)
#       )
#     }
#     
#     taxa_i <- taxa_i %>%
...
#       left_join(., taxa_i, by = c("taxonId", "hierarchy"))
#     
...
# 
#   taxa_filtered <- taxa %>%
#     select(taxonId, name, rank, hierarchy, any_of(shark_taxonomy)) %>%
...
# 
# taxonid_keep <- lapply(double, function(sublist) {
#   sublist[(sublist %in% taxa$taxonId)]
# })
# 
